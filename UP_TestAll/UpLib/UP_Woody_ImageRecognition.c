 ...
 *1:????????Z?????????????2:??????????????
 * @???????????????????????????????????????UP_Woody_Init();?????Woody???
 * @????????????????????????????UP_Woody_StartUp(0x04);????Woody???????????,(?????????????1,0000 0100)
 * @?????????????????????????????UP_Woody_SpeechRecLetIssue();(????woody?????????)
 * @????????????????????????????ID??HSI??????????
 * @??????????????????????????????????????????????????????X??Y??????????????????,????????????>1000??????????????
 * @author   nealwang
 * @date     18/12/2015 
...
#include "UP_Woody_ImageRecognition.h"
 ...
 //static u8 revoic_flage= 0xac;/********oxac ?????woody ????????????*********/
char  UP_Woody_ReturnData1[20]={0x00};//????????????????
 
char  UP_Woody_StartImageRecognition[5]={0x55,0xAA,0x05,0x30,0x34};//?????????????

char  UP_Woody_ClosingImageRecognition[5]={0x55,0xAA,0x05,0x31,0x35};//????????????

char  UP_Woody_ReturnImageRecognition[20]={0x00};//????????????????????

char  UP_Woody_White[4]={0x00,0x01,0x11,0x50};//????????69968

char  UP_Woody_Pink[4]={0x00,0x00,0xc1,0x88};//????????42516
...
u32 UP_Woody_ImagePixel(void)
{
	...
	Pixel=Pixel_0+Pixel_1+Pixel_2+Pixel_3+Pixel_4+Pixel_5+Pixel_6+Pixel_7;//??????
	return Pixel;
	}
	flam=0;	
...
/*************************************************
  ...
  ????????: ??????????????????X??Y???????
  ????????????????: u32 UP_Woody_X_Coordinates(void);u32 UP_Woody_Y_Coordinates(void);void UP_LCD_ClearScreen(void);
  ??????????????: ??    
  ??????????: ??
  ??????????: ??       
  ?????????: ??
...
{   
	...
	UP_Timer_EnableIT(TIMER_CHANNEL2,9000);		 	 //???????2????????9ms
	UP_Timer_Init();
	 if(num>=6)
	{
		num=0;
...
{ 
   ...
   {	
     UP_Timer_DisableIT(TIMER_CHANNEL2);		   /*******????????3???2????*********/
		 number = 0;
		 num = 0;
     flam=0x01;
     Flage=0x01;	
...
/*************************************************
  ????????: UP_Woody_Init
  ????????: ??????????
  ????????????????: UP_EnableUSR232IT(u32 BaudRate);UP_SetUSR232Hadler(void (*ApplicationAddress)(u32));
  ??????????????: ??    
  ??????????: ??
  ??????????: ??       
  ?????????: ??
...
//?????????????????????????????????????????????????????
	...
	UP_Timer_SetHadler(TIMER_CHANNEL2,TimeHadler);			 			 //????????????????????? 
 } 
 /**************************************
	????????: UP_Woody_ImageRecLetIssue
  ????????: ????????????
  ????????????????: UP_USR232_Putc??UP_UART5_Putc
  ??????????????: ??    
  ??????????: ??
  ??????????: ??       
  ?????????: ??
